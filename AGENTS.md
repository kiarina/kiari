...
- `tasks/` — タスク。1 タスク 1 ファイルで、背景・やること・進捗・申し送りを
  ...
  - `tasks/goal/` — ゴール。達成したら終わり、ファイルを消す。今すぐ着手できるか、何かを
    待っているかは入れ替わるので、ディレクトリでは分けず、ファイルの中と「タスク一覧」の見出しで表す
...
踏んだ落とし穴・次の一手など）。今すぐ着手しない作業候補は新しい `tasks/goal/` ファイル、
仕組みとして残す価値のある知見は該当する `docs/`、完了した作業の記録は `HISTORY.md` へ
振り分け、各 `tasks/` ファイルはこれからの作業に保ちます。

## ドキュメントの運用

- ルートと package 内の README、`ARCHITECTURE.md`、`docs/` 以下の技術文書は英語で記述してください。
- `README.ja.md` など、言語別の README は作成しません。
- ユーザーとの相談と、`AGENTS.md`、`tasks/`、`HISTORY.md` の作業管理記録は日本語で構いません。
...
- [kiapi の Web 検索が空結果になる原因を確認する](tasks/goal/kiapi-web-search-empty-result.md)
  — 通常テストが 1 件失敗したままになっている

### 待機中（着手すると決めたら「次に着手・進行中」へ移す）

- [merge 済み config yaml を出力するコマンドを追加する](tasks/goal/config-merge-output-command.md)
