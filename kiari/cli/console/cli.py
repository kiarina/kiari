...
from ._constants.console_option_group import CONSOLE_OPTION_GROUP
from ._decorators.console_options import console_options
from ._i18n import ConsoleI18n
from ._operations.run_console import run_console
from .console_handler import ConsoleRequest

t = get_i18n(ConsoleI18n, get_system_language())

click.rich_click.OPTION_GROUPS["kiari console"] = [
    CONSOLE_OPTION_GROUP,
    *cli.COMMON_OPTION_GROUPS,
]


# fmt: off
@click.command(panel="Run Commands", help=t.command_help)
...
@console_options
@cli.common_options
def console(**kwargs: Any) -> None:
    # fmt: on
    asyncio.run(_console(**kwargs))


async def _console(**kwargs: Any) -> None:
    _apply_stdin_input(kwargs)

    cli_args = cli.build_cli_args(
        extra_args_keys=["texts", "attachments", "stdin_text", "markdown_text"],
    ...
    request = _build_console_request(cli_args.extra_args)
    await cli.run(run_console, profile_name, run_options, request)
...
def _build_console_request(extra_args: cli.ExtraArgs) -> ConsoleRequest | None:
    text = ""
