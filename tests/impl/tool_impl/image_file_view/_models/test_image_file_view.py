...
from kiari.impl.tool_impl.image_file_view import ImageFileView


@pytest.fixture
def tool():
    tool = ImageFileView()
    tool.name = "image_file_view"
    return tool


def _create_ctx(run_context: RunContext, args: dict[str, Any]) -> ToolContext:
    return ToolContext.create(
        tool_call=ToolCall(name="image_file_view", args=args),
        run_context=run_context,
    )


async def test_valid_image_file(
    tool: BaseTool, run_context: RunContext, image_file_path: str
) -> None:
    ctx = _create_ctx(run_context, {"uri_or_file_path": image_file_path})
    ...
    assert content.text == "Viewed the image file successfully."
...
async def test_not_found(tool: BaseTool, run_context: RunContext) -> None:
    ctx = _create_ctx(
        run_context,
        {"uri_or_file_path": "/path/to/non_existent_file.png"},
    )
...
async def test_not_image(tool: BaseTool, run_context: RunContext, text_file_path: str) -> None:
    ...
    content = events[0].message.contents[0]
    assert "is not an image file" in content.text
    assert content.files == []
